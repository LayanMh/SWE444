...
class _GeneratePdfPageState extends State<GeneratePdfPage> {
  ...
  Future<Map<String, dynamic>?> _fetchSwapRequest(String id) async {
    try {
      final doc = await FirebaseFirestore.instance.collection('swap_requests').doc(id).get();
      return doc.data();
    } catch (e) {
  ...
  Future<List<_CourseRow>> _fetchGroupCourses(dynamic groupField, {String? major, dynamic level}) async {
    ...
    try {
      ...
      if (rawSections == null || rawSections.isEmpty) return [];
  ...
  List<String> _normalize(dynamic raw) {
    ...
    if (raw is String) {
      return raw.split(',').map((e) => e.trim()).where((value) => value.isNotEmpty).toList();
    }
    return [];
  }
  ...
  int _sumHours(List<_CourseRow> rows) => rows.fold<int>(0, (total, row) => total + row.hours);

  Future<_FormData> _buildFormData(Map<String, dynamic> req, Map<String, dynamic>? partner) async {
    final major = req['major'] ?? req['Major'] ?? 'Software Engineering';
    final level = req['level'] ?? req['Level'];
    final fromGroup = await _fetchGroupCourses(req['fromGroup'], major: major, level: level);
    final toGroup = await _fetchGroupCourses(req['toGroup'], major: major, level: level);

    final special = (req['specialRequests'] ?? req['SpecialRequests'] ?? {}) as Map;
    final partnerSpecial = (partner?['specialRequests'] ?? partner?['SpecialRequests'] ?? {}) as Map;
    ...
    final partnerHaveSections = _extractSections(partnerSpecial['have'] ?? partner?['have']);
    ...
    final intersect = wantSections.map((e) => e.toLowerCase()).toSet()
    ...
    final additions = [
      ...
      ...partnerHaveCourses.where((c) => intersect.contains(c.section.toLowerCase())),
        ...
        !deletedSet.contains(c.code.toLowerCase())).toList();
    ...
    final sharedRows = fromGroup.where((c) => shared.contains(c.section.toLowerCase())).toList();
  ...
  Future<Uint8List> _buildPdf(_FormData me, _FormData partner) async {
    ...
    void drawForm(_FormData form) {
      ...
      const double baseWidth = 595.0;
      const double baseHeight = 842.0;
      ...
      g.drawImage(bg, ui.Rect.fromLTWH(offsetX, offsetY, drawWidth, drawHeight));
      ...
      void drawField(String text, double baseX, double baseY,
          {double width = 120, double height = 18, PdfFont? font, bool allowBlank = false, PdfStringFormat? format}) {
        g.drawString(
          sanitize(text, allowBlank),
          font ?? regular,
          brush: brush,
          bounds: ui.Rect.fromLTWH(mapX(baseX), mapY(baseY), mapWidth(width), mapHeight(height)),
          format: format,
        );
      }
      ...
      drawField(form.studentMajor, 360, 236, width: 200, height: 40,
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: Column(
          children: [
            // ✅ UPDATED: Header with stars matching profile page
            Container(
            ...
            Expanded(
              child: Container(
                color: kBg,
                child: _loading
                    ? Center(child: CircularProgressIndicator(color: _kTopBarColor))
                    : _pdfBytes == null
                        ? Center(
                            child: Text(
                              _errorMessage ?? 'Tap refresh to generate PDF',
